...
async function enableGoogleForClient(mgmtToken, clientId) {
    ...
    const rc = await fetch(`https://${MGMT_DOMAIN}/api/v2/connections?name=${encodeURIComponent(GOOGLE_CONNECTION_NAME)}`, { headers: { authorization: `Bearer ${mgmtToken}` } });
    if (!rc.ok)
...
async function ensureClientGrant(mgmtToken, clientId, audience, scopes = []) {
    const aud = (audience || "").trim();
    if (!aud || !scopes.length)
        return;
...
function isDcrEventRaw(ev) {
    ...
    return ((type === "sapi" && desc.includes("dynamic client registration")) ||
...
// ---- handler ----
async function handleAuth0LogWebhook(req, res) {
    // Shared-secret auth
    ...
    const ok = auth === `Bearer ${LOG_WEBHOOK_SECRET}` ||
...
export function auth0LogsWebhook() {
    ...
    r.post("/auth0-log-webhook", express.text({ type: "*/*", limit: "2mb" }), (req, res) => {
        ...
        if (typeof req.body === "string") {
            ...
            req.body = looksJson ? req.body : req.body; // handler will parse if needed
        }
