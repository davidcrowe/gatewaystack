...
const FUNCTIONS_BASE = (process.env.FUNCTIONS_BASE ??
    "https://us-central1-<YOUR_PROJECT_ID>.cloudfunctions.net").replace(/\/+$/, "");
...
const RAW_OIDC_DISCOVERY = process.env.OIDC_DISCOVERY ?? `${OAUTH_ISSUER}/.well-known/openid-configuration`;
...
const TOOL_SCOPES = parseToolScopesEnv();
...
function logAuthShape(prefix, req) {
    const { hasAuth, tokenShape, len } = readAuth(req);
    console.log(`[auth:${prefix}] hasAuth=%s tokenShape=%s len=%d path=%s method=%s`, hasAuth, tokenShape, len, req.path, req.method);
}
function logJwtClaims(prefix, token) {
    const dbg = decodeJwtUnsafe(token);
    console.log(`[auth:${prefix}] iss=%s aud=%s scope=%s perms=%s`, dbg?.iss, JSON.stringify(dbg?.aud), dbg?.scope, JSON.stringify(dbg?.permissions));
}
...
function logTokenStructure(prefix, token) {
    const parts = token.split(".");
    const header = parts[0] ? b64urlDecodeToJson(parts[0]) : undefined;
    console.log(`[auth:${prefix}] parts=%d header.alg=%s header.typ=%s`, parts.length, header?.alg, header?.typ);
}
// ---------- Helpers ----------
...
function requireScopes(have, need) {
...
async function subjectToUid(sub, _email) {
    return `auth0:${sub}`;
}
function signGatewayUid(uid) {
    const ts = Math.floor(Date.now() / 1000);
    const base = `${uid}.${ts}`;
    const sig = createHmac("sha256", GATEWAY_HMAC_SECRET).update(base).digest("hex");
    return `${base}.${sig}`;
}
function buildWwwAuthenticate(req) {
    ...
    const scopeParam = (REQUIRED_SCOPES.length
        ? REQUIRED_SCOPES
        : OAUTH_SCOPES.split(" ").filter(Boolean)).join(" ");
...
async function fetchJsonWithRetry(url, tries = 3, timeoutMs = 4000) {
    let lastErr;
...
async function verifyBearer(req) {
    ...
    console.log("[auth:token] segments=%d header.alg=%s header.typ=%s", segments.length, header?.alg, header?.typ);
    // If it’s not a 3-part JWS, give a precise error (don’t just say INVALID_TOKEN_FORMAT)
    if (segments.length !== 3) {
        const e = new Error(segments.length === 5
    ...
    try {
        ...
        const e = new Error("JWT_VERIFY_FAILED");
        e.status = 401;
        e.www = buildWwwAuthenticate(req) + ", error=\"invalid_token\"";
        throw e;
    }
}
async function verifyBearerAndScopes(req, toolName) {
    const payload = await verifyBearer(req);
    const sub = String(payload.sub || "");
    if (!sub) {
        const err = new Error("TOKEN_NO_SUB");
        err.status = 401;
        err.www = buildWwwAuthenticate(req) + ", error=\"invalid_token\"";
        throw err;
    }
...
async function handleMcp(req, res) {
    ...
    if (rpc.method === "tools/call") {
        ...
        try {
            ...
            if (e?.www)
                res.setHeader("WWW-Authenticate", e.www);
            else
...
export async function wellKnownOauthProtectedResource(req, res) {
    ...
    try {
        ...
        const scopes_supported = Array.from(new Set([...OAUTH_SCOPES.split(" ").filter(Boolean), ...REQUIRED_SCOPES]));
...
export async function toolGatewayImpl(req, res) {
    // CORS
    res.setHeader("Access-Control-Allow-Origin", APP_ORIGIN);
    res.setHeader("Vary", "Origin");
    res.setHeader("Access-Control-Allow-Headers", "authorization,content-type,x-request-id");
    ...
    try {
        ...
        if (parts.length !== 3) {
            res.setHeader("WWW-Authenticate", buildWwwAuthenticate(req) + ", error=\"invalid_token\", error_description=\"Expecting JWS access token (3 parts)\"");
...
async function enableGoogleForClient(mgmtToken, clientId) {
    ...
    if (!rc.ok)
        throw new Error(`conn_lookup_http_${rc.status}`);
    const arr = (await rc.json());
    const conn = Array.isArray(arr) ? arr[0] : null;
    if (!conn || !conn.id)
        throw new Error(`connection_not_found:${GOOGLE_CONNECTION_NAME}`);
    // 2) add clientId to enabled_clients (idempotent)
    const enabled = new Set(Array.isArray(conn.enabled_clients) ? conn.enabled_clients : []);
...
function extractClientIdFromLog(ev) {
    return (ev?.data?.client?.client_id ??
        ev?.details?.response?.body?.client_id ??
        ev?.client_id ??
        ev?.data?.client_id ??
        null);
}
function isDcrEvent(ev) {
...
function extractClientIdFromDcr(ev) {
    return (ev?.details?.response?.body?.client_id ?? // common
        ev?.data?.client?.client_id ?? // sometimes present
        ev?.client_id ?? // fallback fields
        ev?.data?.client_id ??
        null);
}
async function findNewestChatGPTClientId(mgmtToken) {
    ...
    const r = await fetch(url, { headers: { authorization: `Bearer ${mgmtToken}` } });
    if (!r.ok) {
        const txt = await r.text();
...
function evtPath(e) {
    return String(e?.details?.request?.path ||
        e?.details?.request?.url ||
        e?.http?.request?.path ||
        "").toLowerCase();
}
function isDcrEventRaw(ev) {
    const e = unwrap(ev);
...
function extractClientIdFromDcrRaw(ev) {
    const e = unwrap(ev);
    return (e?.details?.response?.body?.client_id ??
        e?.client_id ??
        e?.details?.request?.body?.client_id ?? // rare
        null);
}
export async function auth0LogWebhook(req, res) {
...
if (PROXY_TARGET) {
    ...
    toolGatewayRouter.all(`${PROXY_PREFIX}/*`, async (req, res) => {
        try {
            // 1) Verify the Bearer token (or rely on your global gate; if so, you can skip this)
            const payload = await verifyBearer(req);
            const sub = String(payload.sub || "");
            if (!sub) {
                const e = new Error("TOKEN_NO_SUB");
                e.status = 401;
                e.www = buildWwwAuthenticate(req) + ', error="invalid_token"';
                throw e;
            }
            ...
            if (PROXY_INJECT_QUERY)
                urlObj.searchParams.set(PROXY_INJECT_QUERY, sub);
            // 3) Prepare headers/body
            const headers = {};
            for (const [k, v] of Object.entries(req.headers)) {
                if (["host", "connection", "content-length", "transfer-encoding", "authorization"].includes(k))
                    continue;
                headers[k] = Array.isArray(v) ? v.join(", ") : v;
            }
            if (PROXY_INJECT_HEADER)
                headers[PROXY_INJECT_HEADER] = sub;
            ...
            const upstream = await fetch(urlObj.toString(), { method, headers, body, signal: controller.signal });
            clearTimeout(t);
